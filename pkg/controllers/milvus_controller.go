...
func (r *MilvusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	logger := ctrl.LoggerFrom(ctx).WithName("milvus")
...
func (r *MilvusReconciler) ReconcileLegacyValues(ctx context.Context, old, milvus *milvusv1beta1.Milvus) error {
	...
	// Use dynamic logger from context: automatically includes reconcileID
	logger := ctrl.LoggerFrom(ctx)
	logger.Info("SyncValues: " + string(diff))
	err = r.Update(ctx, milvus)
	return err
}
...
func (r *MilvusReconciler) syncLegacyValues(ctx context.Context, m *milvusv1beta1.Milvus) error {
	...
	if !m.Spec.Dep.Etcd.External {
		releaseValues, err := r.helmReconciler.GetValues(ctx, m.Namespace, m.Name+"-etcd")
		if err != nil {
			return err
		}
	...
	case milvusv1beta1.MsgStreamTypePulsar:
		if !m.Spec.Dep.Pulsar.External {
			releaseValues, err := r.helmReconciler.GetValues(ctx, m.Namespace, m.Name+"-pulsar")
			if err != nil {
				return err
			}
	...
	case milvusv1beta1.MsgStreamTypeKafka:
		if !m.Spec.Dep.Kafka.External {
			releaseValues, err := r.helmReconciler.GetValues(ctx, m.Namespace, m.Name+"-kafka")
			if err != nil {
				return err
			}
	...
	if !m.Spec.Dep.Storage.External {
		releaseValues, err := r.helmReconciler.GetValues(ctx, m.Namespace, m.Name+"-minio")
		if err != nil {
			return err
		}
